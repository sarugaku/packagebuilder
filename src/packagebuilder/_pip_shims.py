...
class PipCmd(pip_shims.Command):
    name = "PipCommand"


def build_sources(urls, trusted_hosts):
...
def get_sources():
    pip_command = PipCmd()
    pip_shims.cmdoptions.make_option_group(
        pip_shims.cmdoptions.index_group, pip_command.parser
    )
    parsed, _ = pip_command.parser.parse_args([])
    index_urls = [parsed.index_url] + parsed.extra_index_urls
    sources = build_sources(index_urls, parsed.trusted_hosts)
    return sources
